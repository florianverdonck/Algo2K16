/* Projet algo
 *  Classe permettant de mémoriser les coordonnées gps d'un lieu,
 *  de comparer et de calculer la distance séparant 2 coordonnées
 *  @author O.Legrand
 */
public class Coordonnees implements java.io.Serializable {
	...
	/* latitude du lieu exprimée sous forme d'un entier de type long
	 * Exemple : 3°20’30’’ de latitude Sud = -12030’’
	 * nord = +, sud = - et 3°20’30’’ =  12030’’ car ((3*60)+20)*60)+30
	 */
	private long latitude ;  

	/* longitude du lieu exprimée sous forme d'un entier de type long
	...
	public boolean equals( Coordonnees autresCoordonnées ) {
		return this.latitude == autresCoordonnées.latitude &&
			   this.longitude == autresCoordonnées.longitude;
	}

	/* calcul de la distance séparant 2 coordonnées.
	* calcul minimaliste supposant la terre parfaitement ronde (rayon=6378 km).
	* @param destination coordonnées de l'autre lieu dont on calcule la distance
	* le séparant de l'objet courant. 
	* @return double distance séparant les 2 lieux, exprimée en km.  
	*/
	...
	/* Détermine si deux segments reliant deux paires de coordonnées se croisent ou non.
	* @param p1 coordonnées du premier point du premier segment.
	* @param p2 coordonnées du second point du premier segment.
	* @param q1 coordonnées du premier point du second segment.
	* @param q2 coordonnées du second point du second segment.
	...
	*/
	public static boolean segmentsCroises(Coordonnees p1, Coordonnees p2,
		...
		// Si au moins un des 2 segments est dégénéré (les 2 extrémités sont identiques)
		if(p1.equals(p2)){
			if(q1.equals(q2)){
				if(p1.equals(q1)) return true; // 2 segments dégénérés et identiques
				return false; // 2 segments dégénérés et différents
			}
			// si le segment dégénéré est aligné avec l'autre
		...
		// Si les deux segments sont parallèles
		if((p1Long-p2Long)*(q1Lat-q2Lat) == (p1Lat-p2Lat)*(q1Long-q2Long)){
			// si les ordonnées à l'origine sont identiques
